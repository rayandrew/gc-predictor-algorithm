...
COMBINED_COL = MAIN_COL + STRINGTABLE_COL + PRUNE_COL + OTYRT_COL + TARGET_COL
...
def test_predictor(dataset, main_predictor, stringtable_predictor, prune_predictor, otyrt_predictor):
    from sklearn.metrics import mean_squared_error, r2_score
    X_main = dataset.loc[:, MAIN_COL]
    X_stringtable = dataset.loc[:, STRINGTABLE_COL]
    X_prune = dataset.loc[:, PRUNE_COL]
    X_otyrt = dataset.loc[:, OTYRT_COL]
    ...
    otyrt_y_pred = otyrt_predictor.predict(X_otyrt)
    y_pred = main_y_pred + stringtable_y_pred + prune_y_pred + otyrt_y_pred
...
def generate_diff(dataset, main_predictor, stringtable_predictor, prune_predictor, otyrt_predictor):
    X_main = dataset.loc[:, MAIN_COL]
    X_stringtable = dataset.loc[:, STRINGTABLE_COL]
    X_prune = dataset.loc[:, PRUNE_COL]
    X_otyrt = dataset.loc[:, OTYRT_COL]
    ...
    otyrt_y_pred = np.asarray(otyrt_predictor.predict(X_otyrt), dtype=float)
    pred = main_y_pred + stringtable_y_pred + prune_y_pred + otyrt_y_pred
    diffs = []
...
def main(args):
    ...
    for idx in pbar:
        ...
        mse, r2 = test_predictor(datasets[idx],
                                 main_predictor,
                                 stringtable_predictor,
                                 prune_predictor,
                                 otyrt_predictor)
        pbar.set_description('Generating diffs for dataset {}'.format(name))
        diff = generate_diff(datasets[idx],
                             main_predictor,
                             stringtable_predictor,
                             prune_predictor,
                             otyrt_predictor)
